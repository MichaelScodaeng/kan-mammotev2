...
def train_model(model, t_data, y_true, epochs=3000, lr=2e-4):
...
def run_fitting_analysis():
    ...
    for func_name, y_true in target_functions.items():
        ...
        for i, (model_name, model_instance) in enumerate(models_to_test.items()):
            ...
            if model_name == "B-Spline Expert":
                model = SingleExpertModel(SplineKANLayer, basis_function='b_spline')
            elif model_name == "Fourier Expert":
                model = SingleExpertModel(FourierKANLayer)
            elif model_name == "Wavelet Expert":
...
def train_model_with_loss_return(model, t_data, y_true, epochs=3000, lr=2e-4):
...
def run_interpretability_analysis():
    ...
    train_model(k_mote_model, t_mixed, y_mixed, epochs=3000, lr=2e-4)
...
def run_expert_capability_matrix():
    ...
    for dataset_name, y_data in test_datasets.items():
        ...
        for expert_name, expert_factory in expert_configs.items():
            ...
            final_loss = train_model_with_loss_return(model, t, y_data, epochs=3000, lr=2e-4)
