...
try:
    ...
    from analysis.LeTE import CombinedLeTE
except ImportError as e:
    print(f"ERROR: Could not import required models. Please check your file paths.")
    print(f"Details: {e}")
    sys.exit(1)
...
class ReconstructionModel(nn.Module):
    ...
    It can wrap Time2Vec, CombinedLeTE, or KAN_MAMMOTE models.
    """
    ...
    def forward(self, t_abs, t_rel=None, return_gating_weights=False):
        ...
        elif isinstance(self.encoder, (Time2Vec, CombinedLeTE)):
            # Time2Vec and LeTE baselines only use t_abs
            # For LeTE, we need to squeeze out the feature dimension
            if isinstance(self.encoder, CombinedLeTE):
                embedding = self.encoder(t_abs.squeeze(-1))
            else:
...
def train_model(model, t_abs, y_true, epochs=MAX_EPOCHS, lr=5e-4, patience=400):
...
def plot_figure11_replication(t, y_true, fte_model, lete_model, kan_mammote_model, signal_type):
    """Generates a plot that mimics the layout of LeTE's Figure 11 with three models."""
    fte_model.eval()
    lete_model.eval()
    ...
    with torch.no_grad():
        ...
        y_pred_lete = lete_model(t_abs_in).squeeze().cpu().numpy()
    ...
    residuals_lete = y_true_np - y_pred_lete
    ...
    df = pd.DataFrame({
        ...
        'fte_prediction': y_pred_fte,
        'lete_prediction': y_pred_lete,
        'kan_mammote_prediction': y_pred_kan,
        'fte_residual': residuals_fte,
        'lete_residual': residuals_lete,
        'kan_mammote_residual': residuals_kan
    })
    df.to_csv(f"analysis_figure11_replication/reconstruction_data_{signal_type.lower()}.csv", index=False)
    ...
    axes[0, 0].plot(t_np, y_true_np, 'k-', label='Ground Truth', linewidth=1.5)
    axes[0, 0].plot(t_np, y_pred_fte, 'c--', label='FTE (Time2Vec)', linewidth=1.2)
    axes[0, 0].set_title('FTE (Time2Vec) Reconstruction', fontsize=14)
    axes[0, 0].legend()
    axes[0, 0].grid(True, alpha=0.5, linestyle=':')

    axes[0, 1].plot(t_np, y_true_np, 'k-', label='Ground Truth', linewidth=1.5)
    axes[0, 1].plot(t_np, y_pred_lete, 'b--', label='LeTE', linewidth=1.2)
    axes[0, 1].set_title('LeTE Reconstruction', fontsize=14)
    axes[0, 1].legend()
    axes[0, 1].grid(True, alpha=0.5, linestyle=':')

    axes[0, 2].plot(t_np, y_true_np, 'k-', label='Ground Truth', linewidth=1.5)
    axes[0, 2].plot(t_np, y_pred_kan, 'm--', label='KAN-MAMMOTE', linewidth=1.2)
    axes[0, 2].set_title('KAN-MAMMOTE Reconstruction', fontsize=14)
    axes[0, 2].legend()
    axes[0, 2].grid(True, alpha=0.5, linestyle=':')

    # --- Row 2: Residuals (Error) ---
    axes[1, 0].plot(t_np, residuals_fte, 'r-', label='FTE Residuals', linewidth=1.2)
    ...
    axes[1, 1].plot(t_np, residuals_lete, 'r-', label='LeTE Residuals', linewidth=1.2)
    axes[1, 1].set_title('LeTE Residuals (Error)', fontsize=14)
    axes[1, 1].legend()
    axes[1, 1].grid(True, alpha=0.5, linestyle=':')
    axes[1, 1].set_xlabel("Time (t)")
    
    axes[1, 2].plot(t_np, residuals_kan, 'r-', label='KAN-MAMMOTE Residuals', linewidth=1.2)
    axes[1, 2].set_title('KAN-MAMMOTE Residuals (Error)', fontsize=14)
    axes[1, 2].legend()
    ...
    max_residual = max(np.abs(residuals_fte).max(), np.abs(residuals_lete).max(), np.abs(residuals_kan).max()) * 1.1
    axes[1, 0].set_ylim(-max_residual, max_residual)
    axes[1, 1].set_ylim(-max_residual, max_residual)
    axes[1, 2].set_ylim(-max_residual, max_residual)
    ...
    mse_fte = np.mean(residuals_fte**2)
    mse_lete = np.mean(residuals_lete**2)
    mse_kan = np.mean(residuals_kan**2)
    
    print(f"\n{signal_type} Signal - MSE Comparison:")
    print(f"  FTE (Time2Vec): {mse_fte:.6f}")
    print(f"  LeTE:           {mse_lete:.6f}")
    print(f"  KAN-MAMMOTE:    {mse_kan:.6f}")
    print(f"  Best performer: {['FTE', 'LeTE', 'KAN-MAMMOTE'][np.argmin([mse_fte, mse_lete, mse_kan])]}")
...
if __name__ == '__main__':
    ...
    for signal_type, y_data in datasets.items():
        ...
        # 2. Initialize and Train the LeTE Baseline Model
        print("\n--- Training Baseline: LeTE ---")
        lete_encoder = CombinedLeTE(dim=EMBEDDING_DIM, p=0.5, layer_norm=True, scale=True)
        # Add embedding_dim attribute for compatibility with the wrapper
        lete_encoder.embedding_dim = EMBEDDING_DIM
        lete_model = ReconstructionModel(encoder=lete_encoder)
        train_model(lete_model, t, y_data)

        # 3. Initialize and Train the KAN-MAMMOTE Model
        print("\n--- Training Main Model: KAN-MAMMOTE ---")
        kan_mammote_encoder = KAN_MAMMOTE(
            embedding_dim=EMBEDDING_DIM,
            expert_dim=EMBEDDING_DIM, # Must be a multiple of 16
            num_mixtures=EMBEDDING_DIM,
        ...
        # 4. Generate the comparison plot for this dataset
        plot_figure11_replication(t, y_data, fte_model, lete_model, kan_mammote_model, signal_type)