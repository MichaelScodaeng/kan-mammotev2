...
To address the limitations of prior work, we introduce \textbf{KAN-MAMMOTE}, a novel time encoding framework designed to create a unified and expressive representation from both absolute and relative temporal signals. The architecture is founded on three core principles: (1) specialized, dual-stream encoding to capture the distinct nature of absolute and relative time; (2) the use of highly flexible and mathematically principled function approximators for each stream; and (3) a dynamic fusion mechanism that models the interaction between the two temporal aspects. An overview of the full architecture is presented in Figure~\ref{fig:kan-mammote-architecture}. % Assumes you have a label for your diagram

\begin{figure}[h!]
    \centering
    % You would include your KAN-MAMMOTE diagram here
    % \includegraphics[width=\textwidth]{path/to/kan-mammote-diagram.png}
    \caption{The KAN-MAMMOTE architecture. The model processes absolute time ($t_k$) and relative time ($\Delta t$) through two specialized streams, K-MOTE and SM-Kernel, respectively. The resulting embeddings ($u_k, v_k$) are fused to dynamically control a Mamba (SSM) block, which produces the final, unified temporal representation $T_k$.}
    \label{fig:kan-mammote-architecture}
\end{figure}

\subsection{Dual-Stream Architecture for Temporal Encoding}
Our framework begins by explicitly separating the encoding of absolute and relative time into two parallel streams, reflecting the dual nature of temporal information as outlined in our rationale. For each event $k$ occurring at timestamp $t_k$, we compute the relative time difference $\Delta t_k = t_k - t_{k-1}$ with respect to the immediately preceding event. These two signals, $t_k$ and $\Delta t_k$, are then fed into their respective specialized encoders.

\subsection{Absolute Time Encoding: K-MOTE}
To capture the complex, non-stationary patterns inherent in absolute timestamps (e.g., seasonality, trends, time-of-day effects), we propose the \textbf{Kolmogorov-Arnold Mixture-of-Time-Experts (K-MOTE)} module. This module is built upon Kolmogorov-Arnold Networks (KANs), a new class of neural networks that feature learnable activation functions on their edges, offering superior flexibility and interpretability compared to traditional MLPs with fixed activations.

Recognizing that no single functional basis is optimal for all temporal patterns, K-MOTE employs a mixture-of-experts design. Each expert is a specialized KAN designed to capture a specific type of pattern:
\begin{itemize}
    \item \textbf{Fourier-KAN:} Utilizes Fourier series as its basis functions, making it adept at identifying and modeling periodic patterns (e.g., weekly or daily cycles).
    \item \textbf{Spline-KAN:} Employs B-splines as its basis, allowing it to flexibly capture local, non-periodic trends and irregular shapes.
    \item \textbf{Wavelet-KAN:} Uses wavelet bases to capture transient, bursty patterns or abrupt changes in event frequency.
\end{itemize}
The outputs of these experts are combined to produce a rich embedding $\mathbf{u}_k \in \mathbb{R}^D$ that represents the absolute time context of the event:
\begin{equation}
    \mathbf{u}_k = \text{K-MOTE}(t_k)
\end{equation}

\subsection{Relative Time Encoding: SM-Kernel}
To model the relative time gap $\Delta t_k$, we require a function that is both powerful and adheres to the principle of translation-invariance. For this purpose, we employ a learnable \textbf{Spectral Mixture (SM) Kernel}. Derived from Gaussian Process theory and based on Bochner's theorem, the SM-Kernel models the covariance function as a mixture of Gaussians in the frequency domain.

This choice provides two critical advantages. First, it is \textbf{provably translation-invariant}, correctly modeling the stationary nature of processes like memory decay. Second, it is highly interpretable, as it can automatically discover latent periodicities and aperiodic trends from the data by learning a power spectral density. The SM-Kernel processes the time gap $\Delta t_k$ to produce the relative time embedding $\mathbf{v}_k \in \mathbb{R}^D$:
\begin{equation}
    \mathbf{v}_k = \text{SM-Kernel}(\Delta t_k)
\end{equation}

\subsection{Dynamic Temporal Fusion via a Modified Mamba}
The most significant innovation of our work lies in the fusion of the absolute and relative time embeddings. A simple concatenation or summation would fail to capture the crucial interaction between them (e.g., how the decay of influence over $\Delta t$ might change depending on the absolute time $t_k$).

To model this interaction, we repurpose the selective mechanism of a Mamba architecture, a modern State Space Model (SSM). In a standard Mamba model, the selective state-update parameter $\Delta$ is made input-dependent to filter information along a sequence. We adapt this concept by making it \textbf{time-dependent}.

Specifically, the absolute and relative embeddings, $\mathbf{u}_k$ and $\mathbf{v}_k$, are concatenated and passed through a small multi-layer perceptron (MLP) to generate a dynamic, time-aware selection parameter, $\mathbf{\Delta'}_k$:
\begin{equation}
    \mathbf{\Delta'}_k = \text{MLP}([\mathbf{u}_k \Vert \mathbf{v}_k])
\end{equation}
This parameter $\mathbf{\Delta'}_k$ acts as a learned temporal gate that controls the recurrent state update of a second Mamba block (Mamba2). It allows the model to reason about its own memory, for example, by learning to reset its state (via a large $\mathbf{\Delta'}_k$) after a long period of inactivity, or to persist its state (via a small $\mathbf{\Delta'}_k$) during a burst of events. The final, unified temporal representation $\mathbf{T}_k \in \mathbb{R}^D$ is the output of this Mamba2 block, which has processed the sequence of absolute time embeddings $\mathbf{u}_k$ under the control of the temporal gate $\mathbf{\Delta'}_k$.
\end{document}