...
def run_comprehensive_evaluation(args, trial):
    """
    Run comprehensive evaluation with multiple negative sampling strategies.
    This generates the comprehensive JSON files like you see for TCL.
    
    Note: This is a simplified version that creates placeholder comprehensive results.
    For full comprehensive evaluation, the training script already handles this.
    """
    try:
        # For now, return None to skip comprehensive evaluation in Optuna
        # The full comprehensive evaluation is complex and requires model reloading
        # It's better handled in the main training script
        print(f"Info: Comprehensive evaluation skipped for trial {trial.number} - handled by training script")
        return None
...
def create_objective(dataset='wikipedia', model='TGAT', num_epochs=15, ablation_dir='./optuna_results'):
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        try:
            ...
            # 6. Run comprehensive evaluation after training
            comprehensive_results = None
            try:
                comprehensive_results = run_comprehensive_evaluation(args, trial)
            except Exception as e:
                print(f"Warning: Comprehensive evaluation failed for trial {trial.number}: {e}")
                # Continue without comprehensive results
            ...
            try:
                ...
                # Save basic trial parameters and argparse args for reproducibility
                payload = {
                ...
                # Save comprehensive results if available
                if comprehensive_results is not None:
                    comprehensive_payload = {
                        'primary_strategy': 'random',
                        'time_encoder_type': args.time_encoder_type,
                        'seed': args.seed,
                        'strategies': comprehensive_results
                    }
                    
                    comprehensive_fname = f"{args.save_model_name}_comprehensive_{int(time.time())}.json"
                    comprehensive_path = os.path.join(results_dir, comprehensive_fname)
                    
                    with open(comprehensive_path, 'w') as wf:
                        json.dump(comprehensive_payload, wf, indent=4)
                    print(f"💾 Comprehensive results saved to: {comprehensive_fname}")
            except Exception as e:
