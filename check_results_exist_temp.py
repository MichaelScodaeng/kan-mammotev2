...
def discover_kaga_result_directories(kaga_base_path: str = "./kaga") -> List[str]:
    """
    Discover all directories containing saved_results in the kaga folder structure.
    
    Args:
        kaga_base_path: Base path to the kaga folder
    
    Returns:
        List of paths to directories containing saved_results
    """
    result_dirs = []
    
    if not os.path.exists(kaga_base_path):
        print(f"Warning: Kaga base path {kaga_base_path} does not exist")
        return ["./saved_results"]  # Fallback to default
    
    # Walk through kaga directory structure
    for root, dirs, files in os.walk(kaga_base_path):
        if 'saved_results' in dirs:
            saved_results_path = os.path.join(root, 'saved_results')
            result_dirs.append(saved_results_path)
            #print(f"found results directory: {saved_results_path}")
    
    # Also check for direct model directories (like kaga01/TGAT/)
    for root, dirs, files in os.walk(kaga_base_path):
        # Check if this directory contains model subdirectories with result files
        potential_model_dirs = []
        for dir_name in dirs:
            if dir_name in ALL_MODELS:
                potential_model_dirs.append(dir_name)
        
        if potential_model_dirs:
            # Check if these model directories contain dataset subdirs with JSON files
            for model_dir in potential_model_dirs:
                model_path = os.path.join(root, model_dir)
                if os.path.exists(model_path):
                    # Check if it has dataset subdirs with JSON files
                    for subdir in os.listdir(model_path):
                        ...
                        if os.path.isdir(subdir_path):
                            ...
                            if json_files:
                                ...
                                #print(f"found model directory structure: {root}")
                                break
                    break
    
    # Add default directory if it exists
    default_dir = "./saved_results"
    if os.path.exists(default_dir) and default_dir not in result_dirs:
        result_dirs.append(default_dir)
        #print(f"found default results directory: {default_dir}")
    
    if not result_dirs:
        print("Warning: No results directories found, using default ./saved_results")
        result_dirs = ["./saved_results"]
    
    print(f"\nTotal result directories found: {len(result_dirs)}")
    return result_dirs


def check_evaluation_results_exist(model: str, dataset: str, encoder: str, neg_strategy: str = None, 
                                 kaga_base_path: str = "./kaga") -> Tuple[bool, List[str], Dict]:
    """
    Check if evaluation results already exist for the given combination across all kaga directories.
    ...
    Args:
        ...
        kaga_base_path: Base path to the kaga folder structure
    ...
    result_directories = discover_kaga_result_directories(kaga_base_path)
    
    all_existing_results = []
    all_patterns = []
    
    for base_dir_root in result_directories:
        # Try standard saved_results structure first
        base_dir = f"{base_dir_root}/{model}/{dataset}"
        
        # Also try direct model directory structure (like kaga01/TGAT/dataset)
        if not os.path.exists(base_dir):
            # Check if this is a direct model directory structure
            alt_base_dir = f"{base_dir_root}/{model}/{dataset}"
            if os.path.exists(alt_base_dir):
                base_dir = alt_base_dir
            else:
                # Skip this directory if neither structure exists
                continue
                
        # Look for comprehensive files first (new format that contains all strategies)
        comprehensive_pattern = f"{base_dir}/{model}_{encoder}_seed0_comprehensive_*.json"
        comprehensive_results = glob.glob(comprehensive_pattern)
        
        if comprehensive_results:
            # Found comprehensive file - it contains all strategies
            all_patterns.append(comprehensive_pattern)
            all_existing_results.extend(comprehensive_results)
        else:
            # Fall back to old format
            if neg_strategy is None or neg_strategy == 'random' or neg_strategy == 'combined':
                ...
                raise ValueError(f"Unknown neg_strategy: {neg_strategy}. Must be 'random', 'historical', 'inductive', 'combined', or None.")
            
            all_patterns.append(result_pattern)
            existing_results = glob.glob(result_pattern)
            
            # Filter out validation files if looking for baseline (no _val_ in filename)
            if neg_strategy is not None and neg_strategy != 'combined':
                # Keep only files that match the expected pattern exactly
                if neg_strategy == 'random':
                    # Exclude historical/inductive prefixed files and comprehensive files
                    existing_results = [f for f in existing_results 
                                      ...
                                      and 'comprehensive' not in os.path.basename(f)]
            
            all_existing_results.extend(existing_results)
    
    result_info = {
        'patterns': all_patterns,
        'count': len(all_existing_results),
        'files': all_existing_results,
        'neg_strategy': neg_strategy,
        'searched_directories': result_directories
    }
    
    if all_existing_results:
        # Check if results contain actual evaluation metrics
        return True, all_existing_results, result_info
    
    return False, [], result_info


def get_metrics_from_result_file(model: str, dataset: str, encoder: str, neg_strategy: str = 'random',
                               kaga_base_path: str = "./kaga") -> Optional[Dict]:
    """
    Extract test metrics from result JSON file for the given combination across all kaga directories.
    Handles both old format (separate files) and new comprehensive format (all strategies in one file).
    ...
    for base_dir_root in result_directories:
        ...
        # Also try direct model directory structure
        if not os.path.exists(base_dir):
            alt_base_dir = f"{base_dir_root}/{model}/{dataset}"
            if os.path.exists(alt_base_dir):
                base_dir = alt_base_dir
        ...
        # Look for comprehensive files first (new format)
        comprehensive_pattern = f"{base_dir}/{model}_{encoder}_seed0_comprehensive_*.json"
        comprehensive_results = glob.glob(comprehensive_pattern)
        ...
        if comprehensive_results:
            ...
            # Fall back to old format
            if neg_strategy == 'random' or neg_strategy == 'combined':
    ...
    result_file = all_existing_results[0]
    ...
    try:
        ...
        if "strategies" in data and neg_strategy in data["strategies"]:
            ...
            if "transductive_test" in strategy_data:
                trans_metrics = strategy_data["transductive_test"]["metrics"]
                metrics["test_average_precision"] = trans_metrics.get("average_precision")
                metrics["test_roc_auc"] = trans_metrics.get("roc_auc")
                metrics["test_loss"] = strategy_data["transductive_test"].get("loss")
            
            # Extract inductive (new node) test metrics
            if "inductive_test" in strategy_data:
                ind_metrics = strategy_data["inductive_test"]["metrics"]
                metrics["new_node_test_average_precision"] = ind_metrics.get("average_precision")
                metrics["new_node_test_roc_auc"] = ind_metrics.get("roc_auc")
                metrics["new_node_test_loss"] = strategy_data["inductive_test"].get("loss")
                
        else:
            # Old format - fallback
            if "test metrics" in data:
                test_metrics = data["test metrics"]
                metrics["test_average_precision"] = test_metrics.get("average_precision")
                metrics["test_roc_auc"] = test_metrics.get("roc_auc")
            
            if "new node test metrics" in data:
                new_node_metrics = data["new node test metrics"]
                metrics["new_node_test_average_precision"] = new_node_metrics.get("average_precision")
                metrics["new_node_test_roc_auc"] = new_node_metrics.get("roc_auc")
            
            # Also include validation metrics for completeness
            if "validate metrics" in data:
                val_metrics = data["validate metrics"]
                metrics["validate_average_precision"] = val_metrics.get("average_precision")
                metrics["validate_roc_auc"] = val_metrics.get("roc_auc")
                
            if "new node validate metrics" in data:
                new_node_val_metrics = data["new node validate metrics"]
                metrics["new_node_validate_average_precision"] = new_node_val_metrics.get("average_precision")
                metrics["new_node_validate_roc_auc"] = new_node_val_metrics.get("roc_auc")
...
def create_completion_matrix(combinations: List, check_func, matrix_name: str, output_dir: str) -> pd.DataFrame:
    ...
    # Fill matrix based on completion status with tqdm progress
    for combo in tqdm(combinations, desc=f"Checking {matrix_name}", unit="combo"):
...
def create_comprehensive_results_table(models: List[str], datasets: List[str], 
                                      ...
                                      output_dir: str, kaga_path: str = "./kaga") -> pd.DataFrame:
    """
    ...
    # Create all combinations for progress tracking
    all_combinations = [(model, dataset, encoder, neg_strategy) 
    ...
    pbar = tqdm(all_combinations, desc="Processing combinations", unit="combo")
    
    for model, dataset, encoder, neg_strategy in pbar:
        # Get metrics for this combination
        metrics = get_metrics_from_result_file(model, dataset, encoder, neg_strategy, kaga_path)
        ...
        if metrics:
            ...
            # Extract all metrics - these are now individual keys
            for key, value in metrics.items():
                if key.startswith(('test_', 'new_node_test_', 'validate_', 'new_node_validate_')):
                    try:
        ...
        pbar.set_postfix({'Found': found, 'Rate': f'{found/len(pbar)*100:.1f}%' if len(pbar) > 0 else '0%'})
...
def create_time_encoder_metrics_matrix(time_encoder: str, models: List[str], datasets: List[str], 
    ...
    # Create all combinations for progress tracking
    all_combinations = [(model, dataset) for model in sorted(models) for dataset in sorted(datasets)]
    
    for model, dataset in tqdm(all_combinations, desc=f"Extracting metrics for {time_encoder}", unit="combo"):
        if model not in metrics_data:
            metrics_data[model] = {}
        
        metrics = get_metrics_from_result_file(model, dataset, time_encoder, kaga_base_path=kaga_path)
        if metrics:
...
def generate_summary_stats(combinations: List, check_func, output_dir: str) -> Dict:
    ...
    for combo in tqdm(combinations, desc="Generating summary stats", unit="combo"):
        model, dataset, encoder, neg_strategy = combo
...
def parse_arguments():
    ...
    parser.add_argument('--kaga_path', type=str, default='./kaga',
                        help='Path to kaga folder containing distributed results (default: ./kaga)')
...
if __name__ == "__main__":
    ...
    print("🔍 Experiment Completion Analysis (with Kaga Support)")
    print(f"Analysis started at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"Output directory: {args.output_dir}")
    print(f"Kaga path: {args.kaga_path}")
    ...
    # Discover available result directories
    print(f"\n🔍 Discovering result directories in {args.kaga_path}...")
    result_dirs = discover_kaga_result_directories(args.kaga_path)
    ...
    if args.time_encoder:
        ...
        time_encoder_matrix = create_time_encoder_completion_matrix(
            ...
            check_func=lambda m, d, e, n: check_evaluation_results_exist(m, d, e, n, args.kaga_path),
    ...
    check_func_with_kaga = lambda m, d, e, n: check_evaluation_results_exist(m, d, e, n, args.kaga_path)
    
    matrices['model_dataset'] = create_completion_matrix(
        unique_combinations, 
        check_func_with_kaga, 
        "Model-Dataset", 
        args.output_dir
    )
    ...
    summary = generate_summary_stats(unique_combinations, check_func_with_kaga, args.output_dir)
    ...
    if args.verbose and summary['failed_combinations']:
        ...
        for i, combo in enumerate(summary['failed_combinations']):
            ...
            completed, files, info = check_evaluation_results_exist(model, dataset, encoder, kaga_base_path=args.kaga_path)
            print(f"   {i+1:3d}. {model:12s} + {dataset:12s} + {encoder:20s}")
            if args.verbose:
                print(f"        Patterns: {info['patterns']}")
                print(f"        Found files: {info['count']}")
                print(f"        Searched directories: {len(info['searched_directories'])}")
                print()