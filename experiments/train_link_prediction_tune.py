...
import optuna  # ← MODIFICATION 1: Add Optuna import
...
# <<<< MODIFICATION 2: Create a new function to wrap the main logic >>>>
def run_training_session(args=None, trial=None):
    """
    Main training session that can be called either standalone or with Optuna trial.
    
    Args:
        args: Training arguments from get_link_prediction_args() (optional - will be created if None)
        trial: Optional Optuna trial object for hyperparameter optimization
        
    Returns:
        float: Best validation average precision score
    """
    warnings.filterwarnings('ignore')

    # get arguments only if not provided (for backward compatibility)
    if args is None:
    ...
    for run in range(start_seed, args.num_runs):
        ...
        # <<<< MODIFICATION 3: Ensure each trial has a unique output directory >>>>
        # This prevents multiple trials from overwriting each other's models and logs.
        if trial:
            # Append trial number to the save name suffix
            trial_suffix = f"trial_{trial.number}"
            if hasattr(args, 'save_model_name_suffix') and args.save_model_name_suffix:
                args.save_model_name_suffix = f"{args.save_model_name_suffix}_{trial_suffix}"
            else:
                args.save_model_name_suffix = trial_suffix
        ...
        for epoch in range(start_epoch, args.num_epochs):
            ...
            # <<<< MODIFICATION 4: The Core Optuna Integration >>>>
            # We add this block right after validation is complete for the epoch.
            if trial:
                # 1. Extract the metric you want to optimize.
                val_metrics_avg = {k: np.mean([m[k] for m in val_metrics]) for k in val_metrics[0].keys()}
                current_val_ap = val_metrics_avg.get('average_precision', 0.0)
                
                # Keep track of the best score seen so far in this trial
                if not hasattr(trial, '_best_val_ap'):
                    trial._best_val_ap = 0.0
                if current_val_ap > trial._best_val_ap:
                    trial._best_val_ap = current_val_ap

                # 2. Report the intermediate value to Optuna.
                trial.report(current_val_ap, epoch)

                # 3. Handle pruning.
                if trial.should_prune():
                    # If Optuna says this trial is not promising, stop it.
        ...
        logger.info(f'Evaluating with PRIMARY negative sampling strategy: {args.negative_sample_strategy}')
        ...
        logger.info(f'save model results at {save_result_path}')
        
        # ========================================================================
        # 🔍 COMPREHENSIVE EVALUATION: Test all negative sampling strategies
        # ========================================================================
        logger.info("\n" + "="*80)
        logger.info("🔍 COMPREHENSIVE EVALUATION: Testing all negative sampling strategies")
        logger.info("="*80)
        
        all_strategies = ['random','historical', 'inductive']
        comprehensive_results = {
            'primary_strategy': args.negative_sample_strategy,
            'time_encoder_type': args.time_encoder_type,
            'seed': run,
            'strategies': {}
        }
        ...
        for strategy in all_strategies:
            logger.info(f"\n{'─'*80}")
            logger.info(f"📊 Testing negative sampling strategy: {strategy}")
            logger.info(f"{'─'*80}")
            
            # Create negative samplers for this strategy
            if strategy != 'random':
                test_neg_sampler_strategy = NegativeEdgeSampler(
                ...
                new_node_test_neg_sampler_strategy = NegativeEdgeSampler(
            ...
            # Backup memory if needed
            if args.model_name in ['JODIE', 'DyRep', 'TGN']:
                val_backup_memory_bank = model[0].memory_bank.backup_memory_bank()
            
            # Evaluate on transductive test set
            strategy_test_losses, strategy_test_metrics = evaluate_model_link_prediction(
                model_name=args.model_name,
                model=model,
                neighbor_sampler=full_neighbor_sampler,
                evaluate_idx_data_loader=test_idx_data_loader,
                evaluate_neg_edge_sampler=test_neg_sampler_strategy,
                evaluate_data=test_data,
                loss_func=loss_func,
                num_neighbors=args.num_neighbors,
                time_gap=args.time_gap,
            ...
            strategy_new_node_test_losses, strategy_new_node_test_metrics = evaluate_model_link_prediction(
                model_name=args.model_name,
                model=model,
                neighbor_sampler=full_neighbor_sampler,
                evaluate_idx_data_loader=new_node_test_idx_data_loader,
                evaluate_neg_edge_sampler=new_node_test_neg_sampler_strategy,
                evaluate_data=new_node_test_data,
                loss_func=loss_func,
                num_neighbors=args.num_neighbors,
                time_gap=args.time_gap,
            ...
            strategy_test_metrics_avg = {
                k: np.mean([m[k] for m in strategy_test_metrics]) 
                for k in strategy_test_metrics[0].keys()
            }
            strategy_new_node_test_metrics_avg = {
                k: np.mean([m[k] for m in strategy_new_node_test_metrics]) 
                for k in strategy_new_node_test_metrics[0].keys()
            }
            
            # Log results
            logger.info(f"\n{strategy.upper()} Negative Sampling Results:")
            logger.info(f"  Transductive Test Loss: {np.mean(strategy_test_losses):.4f}")
            for metric_name, value in strategy_test_metrics_avg.items():
                logger.info(f"  Transductive Test {metric_name}: {value:.4f}")
            
            logger.info(f"  Inductive Test Loss: {np.mean(strategy_new_node_test_losses):.4f}")
            for metric_name, value in strategy_new_node_test_metrics_avg.items():
                logger.info(f"  Inductive Test {metric_name}: {value:.4f}")
            
            # Store results
            comprehensive_results['strategies'][strategy] = {
                'transductive_test': {
                    'loss': float(np.mean(strategy_test_losses)),
                    'metrics': {k: float(v) for k, v in strategy_test_metrics_avg.items()}
                },
                'inductive_test': {
                    'loss': float(np.mean(strategy_new_node_test_losses)),
                    'metrics': {k: float(v) for k, v in strategy_new_node_test_metrics_avg.items()}
                }
            }
        
        # Save comprehensive results
        comprehensive_results_json = json.dumps(comprehensive_results, indent=4)
        comprehensive_save_path = os.path.join(
            save_result_folder, 
            f"{args.save_model_name}_comprehensive_{timestamp}.json"
        )
        with open(comprehensive_save_path, 'w') as f:
            f.write(comprehensive_results_json)
        
        logger.info(f"\n{'='*80}")
        logger.info(f"✅ Comprehensive evaluation results saved to:")
        logger.info(f"   {comprehensive_save_path}")
        logger.info(f"{'='*80}\n")
        
        # Print comparison table
        logger.info("\n" + "="*80)
        logger.info("📊 COMPARISON TABLE: Negative Sampling Strategies")
        logger.info("="*80)
        logger.info(f"{'Strategy':<15} {'Trans AP':<12} {'Trans AUC':<12} {'Ind AP':<12} {'Ind AUC':<12}")
        logger.info("─"*80)
        for strategy in all_strategies:
            trans_ap = comprehensive_results['strategies'][strategy]['transductive_test']['metrics'].get('average_precision', 0)
            trans_auc = comprehensive_results['strategies'][strategy]['transductive_test']['metrics'].get('roc_auc', 0)
            ind_ap = comprehensive_results['strategies'][strategy]['inductive_test']['metrics'].get('average_precision', 0)
            ind_auc = comprehensive_results['strategies'][strategy]['inductive_test']['metrics'].get('roc_auc', 0)
            
            marker = " ⭐" if strategy == args.negative_sample_strategy else ""
            logger.info(f"{strategy:<15} {trans_ap:<12.4f} {trans_auc:<12.4f} {ind_ap:<12.4f} {ind_auc:<12.4f}{marker}")
        logger.info("="*80 + "\n")
        # ========================================================================
    ...
    # <<<< MODIFICATION 5: Return the best validation score >>>>
    # If the trial was not pruned, it finishes. We need to return its best score.
    if trial:
        # For Optuna trials, return the best validation AP seen during training
        final_val_ap = getattr(trial, '_best_val_ap', 0.0)
        logger.info(f'Trial {trial.number} completed with best validation AP: {final_val_ap:.4f}')
        return final_val_ap
    else:
        # For normal runs, we don't need to return anything
        return None


# <<<< MODIFICATION 6: The original main block now just calls the function >>>>
# This ensures the script can still be run directly from the command line.
if __name__ == "__main__":
    # get arguments
    args = get_link_prediction_args(is_evaluation=False)
    # Call the main function without a trial object
    run_training_session(args, trial=None)
