...
from models.time_encoders.ablation_encoders import (
    KMOTEAbsOnly, KMOTERelOnly
)
from models.time_encoders.kmm import KMM
...
class BadgePredictionModel(nn.Module):
    ...
    def _needs_both_times(self, encoder_type):
        ...
        dual_time_encoders = [
            'KMM',
            'k_mote_abs', 'k_mote_rel',   # K-MOTE variants also need both times
...
def collect_experiment_parameters(args, encoder_name, dataset_info):
    ...
    if 'k_mote' in encoder_name or 'KMM' in encoder_name:
        experiment_params['k_mote_parameters'] = {
            'expert_dim': args.expert_dim,
            'wavelet_type': args.wavelet_type
        }
    
    if 'KMM' in encoder_name:
        experiment_params['KMM_parameters'] = {
            'mamba_d_state': args.mamba_d_state,
            'mamba_d_conv': args.mamba_d_conv,
            'mamba_expand': args.mamba_expand,
    ...
    if encoder_name in ['lete_rel', 'lete_relative',
                       'fte_rel', 'fte_relative', 'k_mote_rel',
                       'KMM']:
