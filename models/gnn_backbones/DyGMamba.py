...
class DyGMamba(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
                 ...
                 dropout: float = 0.1,gamma: float = 0.5, max_input_sequence_length: int = 512, max_interaction_times: int = 10, device: str = 'cpu', time_encoder: nn.Module = None):
        """
        DyGMamba model.
        ...
        if time_encoder is not None:
            ...
            print(f"DyGMamba: Using custom time encoder: {type(time_encoder).__name__}")
        else:
            self.time_encoder = TimeEncoder(time_dim=time_feat_dim)
            print(f"DyGMamba: Using default TimeEncoder")
    ...
    def time_modeling(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, src_node_interact_times: np.ndarray,
        ...
        if is_kan_mammote:
            ...
            if should_debug_model() and not hasattr(self, '_debug_printed_sorting_timediff') and padded_abs_time.size > 0:
                ...
                first_abs_times = padded_abs_time[0]
                is_sorted = all(first_abs_times[i] <= first_abs_times[i+1] for i in range(len(first_abs_times)-1))
                print(f"   First row absolute times: {first_abs_times[:5]}...")
                print(f"   Is chronologically sorted: {is_sorted}")
                
                # Check t_abs and t_rel values
                print(f"   t_abs sample: {t_abs[0, :5].cpu().numpy()}")
                print(f"   t_rel sample (time gaps): {t_rel[0, :5].cpu().numpy()}")
                
                self._debug_printed_sorting_timediff = True
    ...
    def get_features(self, node_interact_times: np.ndarray, padded_nodes_neighbor_ids: np.ndarray, padded_nodes_edge_ids: np.ndarray,
        ...
        if is_kan_mammote:
            ...
            if should_debug_model() and not hasattr(self, '_debug_printed_sorting_neighbor') and padded_nodes_neighbor_times.size > 0:
                ...
                self._debug_printed_sorting_neighbor = True
