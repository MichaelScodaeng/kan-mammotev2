...
class WaveletKANLayer(nn.Module):
    ...
    MEMORY-OPTIMIZED: Processes features sequentially to avoid large tensor expansions.
    """
    ...
    def forward(self, t: torch.Tensor) -> torch.Tensor:
        """
        MEMORY-OPTIMIZED: Process each input feature dimension sequentially
        to avoid creating large intermediate tensors.
        """
        if t.dim() == 2: 
            t = t.unsqueeze(-1)
        ...
        wavelet_features = torch.zeros(
            batch_size, seq_len, input_dim * self.n_wavelets,
            device=t.device, dtype=t.dtype
        )
        
        # Process each input dimension separately to save memory
        for i in range(input_dim):
            # Extract single feature: (batch_size, seq_len)
            t_i = t[:, :, i]
            
            # Get parameters for this feature
            scales_i = F.softplus(self.scales[i]) + 0.1  # (n_wavelets,)
            shifts_i = self.shifts[i]  # (n_wavelets,)
            
            # Process each wavelet sequentially
            for j in range(self.n_wavelets):
                # Compute wavelet input for this specific scale/shift
                # (batch_size, seq_len)
                wavelet_input_ij = (t_i - shifts_i[j]) / scales_i[j]
                
                # Compute wavelet activation based on type
                if self.wavelet_type == 'adaptive_morlet':
                    freq_ij = self.frequencies[i, j]
                    sharp_ij = self.sharpness[i, j]
                    activation = self.adaptive_morlet_wavelet(wavelet_input_ij, freq_ij, sharp_ij)
                    ...
                    asym_ij = self.asymmetry[i, j]
                    steep_ij = self.steepness[i, j]
                    activation = self.shock_wavelet(wavelet_input_ij, asym_ij, steep_ij)
                else:
