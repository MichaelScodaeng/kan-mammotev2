...
class SplineKANLayer(nn.Module):
    ...
    def __init__(self, input_dim: int, output_dim: int, grid_size: int = 8, basis_function: str = 'b_spline'):
        ...
        else:  # RBF implementation (optimized, LeTE-style)
            # ===== OPTIMIZATION 1: Shared RBF centers across dimensions =====
            # Instead of (input_dim, output_dim, grid_size), use (grid_size,) or (1, grid_size)
            # This reduces parameters from input_dim × output_dim × grid_size to just grid_size!
            
            # Initialize centers across a wider, learnable range
            internal_range = [-3, 3]  # Wider than [-2, 2] to handle diverse inputs
            centers_init = torch.linspace(internal_range[0], internal_range[1], grid_size, dtype=torch.float32)
            self.centers = nn.Parameter(centers_init.unsqueeze(0))  # Shape: (1, grid_size)
            
            # ===== OPTIMIZATION 2: Learnable width parameter (gamma) =====
            # Use a single gamma per RBF center (not per input_dim × output_dim combo)
            # Initialize with reasonable default (1/spacing between centers)
            spacing = (internal_range[1] - internal_range[0]) / (grid_size - 1)
            gamma_init = 1.0 / (spacing ** 2)  # Standard RBF initialization
            self.gammas = nn.Parameter(torch.full((1, grid_size), gamma_init))  # Shape: (1, grid_size)
            
            # ===== OPTIMIZATION 3: Direct RBF-to-output mapping =====
            # Map (batch, seq_len, grid_size) → (batch, seq_len, output_dim)
            # This replaces the inefficient input_dim * grid_size intermediate
            self.rbf_weights = nn.Parameter(
                torch.randn(grid_size, output_dim) / math.sqrt(grid_size)
            )
            self.rbf_bias = nn.Parameter(torch.zeros(output_dim))
            
            # Optional: Add a global linear component (like LeTE's base_weight)
            self.rbf_base = nn.Parameter(torch.randn(input_dim, output_dim) / math.sqrt(input_dim))
...
class KMOTE(nn.Module):
    ...
    def __init__(self, input_dim: int, output_dim: int,
        ...
        self.time_linear_transform = nn.Linear(input_dim, output_dim//2)
        ...
        self.experts = nn.ModuleList([
            SplineKANLayer(output_dim//2, output_dim, basis_function='b_spline', grid_size=8),
            FourierKANLayer(output_dim//2, output_dim, n_harmonics=8),
            WaveletKANLayer(output_dim//2, output_dim, n_wavelets=8, wavelet_type=wavelet_type),
            SplineKANLayer(output_dim//2, output_dim, basis_function='rbf', grid_size=8)
        ])
        # ===========================================================================
        ...
        self.gating_network = nn.Sequential(
            nn.Linear(output_dim//2, 64), # Input is now output_dim
            nn.GELU(),
    ...
    def _initialize_time_transform(self):
        ...
        with torch.no_grad():
            ...
            frequencies = 1.0 / (10 ** torch.linspace(0, 9, self.output_dim//2, dtype=torch.float32))
