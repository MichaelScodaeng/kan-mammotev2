...
class SplineKANLayer(nn.Module):
    ...
    def __init__(self, input_dim: int, output_dim: int, grid_size: int = 5, basis_function: str = 'b_spline', order: int = 3, grid_range: list = [-1, 1]):
        ...
        if self.basis_function == 'b_spline':
            ...
            # Base weight for the linear+activation branch (improved initialization like LeTE)
            self.base_weight = nn.Parameter(torch.Tensor(self.input_dim, self.output_dim))
            
            # Spline coefficients (improved initialization like LeTE)
            self.spline_weight = nn.Parameter(
                torch.Tensor(self.input_dim, self.output_dim, self.grid_size + self.order)
            )
            
            # Initialize parameters properly (following LeTE's best practices)
            self._initialize_spline_parameters()
    ...
    def _initialize_spline_parameters(self):
        """
        Initialize B-spline parameters following LeTE's best practices.
        This provides better numerical stability and training convergence.
        
        Uses:
        - Kaiming uniform initialization for base_weight (better for tanh activation)
        - Normal initialization with controlled std for spline_weight (stable B-spline coefficients)
        """
        import math
        # Initialize base weight with Kaiming uniform (optimal for tanh activation)
        nn.init.kaiming_uniform_(self.base_weight, a=math.sqrt(5))
        
        # Initialize spline weight with controlled normal distribution
        # This prevents exploding gradients in B-spline coefficients
