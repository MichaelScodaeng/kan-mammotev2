...
class KAN_MAMMOTE(nn.Module):
    ...
    def __init__(self, embedding_dim: int, expert_dim: int, num_mixtures: int, 
        ...
        self.k_mote_abs = KMOTE(
            ...
            time_type='absolute',  # Specify time type for proper initialization
            use_scale=True,       # Enable scale parameter like LeTE
            version='v2'          # Use fixed B-spline implementation
        )
        ...
        if use_kmote_for_relative:
            ...
            self.k_mote_rel = KMOTE(
                ...
                output_dim=expert_dim, 
                wavelet_type=wavelet_type,
                time_type='relative',  # Specify time type for proper initialization
                use_scale=True,        # Enable scale parameter like LeTE
                version='v2'           # Use fixed B-spline implementation
            )
            self.sm_kernel = None
