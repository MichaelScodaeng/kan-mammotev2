...
class DynamicMambaSSM(Mamba2):
    ...
    def forward(self, u: torch.Tensor, delta_t_embedding: torch.Tensor,
        ...
        if self.use_mem_eff_path and inference_params is None:
            ...
            #print(f"Shape of zxbcdt_modified BEFORE KERNEL CALL: {zxbcdt_modified.shape}") # THIS SHOULD BE (batch*seqlen, d_in_proj)
            ...
            zxbcdt_modified = zxbcdt_modified.contiguous()
        ...
        # --- Path 2: Unfused path or inference (not use_mem_eff_path) ---
            ...
            if causal_conv1d_fn is None or self.activation not in ["silu", "swish"]:
                ...
                xBC_transposed_contiguous = xBC.transpose(1, 2).contiguous() # <--- ADD .contiguous() HERE
    ...
    def step(self, hidden_states: torch.Tensor, conv_state: torch.Tensor, ssm_state: torch.Tensor,
        ...
        if selective_state_update is None:
            ...
            kernel_dt_bias_for_kernel = repeat(self.dt_bias.contiguous().view(self.nheads), "h -> h p", p=self.headdim) # (nheads, headdim)
